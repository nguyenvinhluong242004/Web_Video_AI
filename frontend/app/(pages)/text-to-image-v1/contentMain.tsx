      ...
      <div className="flex flex-col md:flex-row border-b-2 pb-12 md:pb-3 border-gray-600">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className={`grid ${images[activeIndex].length > 1 ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-1'} gap-3`}>
                ...
                <div key={i} className="relative group cursor-pointer" onClick={() => toggleSelectImage(img)}>
                  <img
                    ...
                    className={`w-full h-32 object-cover rounded-md border-2 ${
                      allImages.includes(img) ? "border-blue-500" : "border-gray-300"
                    }`}
                  />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {allImages.map((img, i) => (
              <img
                ...
                src={img}
                alt={`selected-${i}`}
                className="w-full h-28 object-cover rounded-md border border-green-500"
              />
            ))}
          </div>
        </div>
